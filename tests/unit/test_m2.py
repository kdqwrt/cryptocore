...
class TestMilestone2:
    ...
    def test_ecb_error_handling(self):
        ...
        try:
            ...
            assert "multiple of block size" in str(e)
    ...
    def test_iv_handling(self):
        ...
        try:
            ...
            assert "IV must be 16 bytes" in str(e)
    ...
    def test_short_file_error(self):
        ...
        try:
            try:
                ...
                assert "too short to contain IV" in str(e)
        finally:
            if os.path.exists(temp_file):
                os.unlink(temp_file)
    ...
    def test_openssl_compatibility_ecb(self):
        if not self._check_openssl_available():
            print("SKIPPED: OpenSSL not available")
            return
        ...
        try:
            ...
            print(f"SKIPPED: OpenSSL failed: {e}")
        finally:
...
def run_milestone2_tests():
    ...
    for method_name in sorted(test_methods):
        ...
        try:
            ...
            # Проверяем, не является ли это "пропуском" (SKIP)
            if "SKIPPED" in str(e) or hasattr(e, 'args') and "SKIPPED" in str(e.args):
                print(f"SKIPPED")
                skipped += 1
            else:
                print(f"ERROR: {e}")
